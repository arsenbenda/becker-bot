...
def extract_probability_gpt(question: str, research: str, market_price: float) -> dict | None:
    ...
    prompt = (
        ...
        f"CRITICAL CALIBRATION RULES — you MUST follow these:\n\n"
        f"PROBABILITY SCALE (read carefully):\n"
        f"- 0.95-0.99: NEAR-CERTAIN. Only use when failure requires multiple unlikely events. "
        f"Example: team 15+ points clear with 3 games left, or election with 20+ point polling lead.\n"
        f"- 0.85-0.94: VERY LIKELY. Strong favorite, but plausible upset paths exist. "
        f"Example: team 8-12 points clear with 8+ games left, or candidate with 10-15 point lead.\n"
        f"- 0.70-0.84: LIKELY. Clear favorite but meaningful uncertainty remains. "
        f"Example: team leading league by 5-8 points, frontrunner with 5-10 point lead.\n"
        f"- 0.55-0.69: LEAN YES. Evidence favors YES but outcome is contested.\n"
        f"- 0.40-0.54: TOSS-UP. Genuinely uncertain, evidence balanced.\n"
        f"- 0.20-0.39: LEAN NO. Evidence favors NO but not impossible.\n"
        f"- 0.05-0.19: UNLIKELY. Would require significant surprise or reversal.\n"
        f"- 0.01-0.04: NEAR-IMPOSSIBLE. Supernatural, no evidence, no realistic path.\n\n"
        f"CATEGORY RULES:\n"
        f"- Supernatural/physically impossible events: 0.01-0.02\n"
        f"- Single team winning 20+ team tournament: favorites 0.15-0.25, mid-tier 0.05-0.15, longshots 0.01-0.05\n"
        f"- Political removal/resignation with no active proceedings: 0.02-0.10\n"
        f"- Relegation of an established top-flight team: base rate 0.05-0.15 unless currently in bottom 3\n"
        f"- NEVER give two teams in the same league/tournament both >0.90\n\n"
        f"ANCHORING RULE: The current market price reflects the wisdom of thousands of traders. "
        f"If your estimate differs from market price by more than 15 percentage points, "
        f"you MUST explain specifically what information the market is missing.\n"
        f"If new information has emerged since the market last moved meaningfully, "
        f"weight it heavily. Otherwise, be skeptical of your deviation from price.\n\n"
        f"DATA USAGE: If the research includes betting odds, polling numbers, or market prices, "
        f"use them as primary anchors — not just supporting evidence.\n"
        f"- You MUST commit to a specific number — do not hedge.\n\n"
        f"Market question: {question}\n"
